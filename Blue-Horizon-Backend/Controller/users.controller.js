    ...
    usersRegister: async function(req, res){
        try{
            ...
            if(req.file){
                ...
                const data = await userServices.usersRegister(name , username, email, password, bio, profilePicture , profilePictureId);
                if(data){
                    res.status(200).json({
        ...
        try{
            if(!userId){
                ...
                if(!name || !username  || !bio || !profile_picture || !profile_picture_id){
                    ...
                    const data = await userServices.updateUser(Number(userId) , name , username,  bio , profile_picture, profile_picture_id);
                    if(data){
                        res.status(200).json({
                            status: status.SUCCESS,
